...
class Entity:
    ...
    def last_changed(self) -> typing.Union[None, datetime.datetime]:
        return (
            datetime.datetime.fromisoformat(self.data["last_changed"])
            if "last_changed" in self.data.keys()
            else None
        )
    ...
    def last_updated(self) -> typing.Union[None, datetime.datetime]:
        return (
            datetime.datetime.fromisoformat(self.data["last_updated"])
    ...
    def area_id(self) -> typing.Union[str, None]:
        ...
        return raw_resp if raw_resp != "None" else None
...
class HomeAssistant:
    ...
    def entities(self) -> dict[str, Entity]:
        ...
        if response.status_code == 200:
            return {
                entity["entity_id"]: Entity(self, entity) for entity in response.json()
            }
        else:
    ...
    def call_template(self, template_function: str) -> typing.Any:
        response = self.session.post(
            self.url("/template"), json={"template": "{{ " + template_function + " }}"}
        )
        if response.status_code == 200:
    ...
    def mapped_template(self, mapping: dict[str, str]) -> dict[str, str]:
        ...
        if response.status_code == 200:
            ...
            for item in response.text.split("&"):
                try:
                    items[item.split("=", maxsplit=1)[0]] = eval(
                        item.split("=", maxsplit=1)[1]
                    )
                except:
                    items[item.split("=", maxsplit=1)[0]] = item.split("=", maxsplit=1)[
                        1
                    ]
            return items
        else:
    ...
    def area(self, name: str) -> Area:
        raw_output = self.mapped_template(
            {
                "name": f"area_name('{name}')",
                "entities": f"area_entities('{name}')",
                "devices": f"area_devices('{name}')",
            }
        )
        raw_output["id"] = name
        return Area(self, raw_output) if raw_output["name"] else None

    def get_config(self) -> Config:
        response = self.session.get(self.url("/config"))
        if response.status_code == 200:
